document.addEventListener('DOMContentLoaded', function () {
    ...
    // ✅ Lấy tất cả UID từ Firestore collection "users"
    const userList = document.getElementById('user-uid-list');
    if (userList) {
        firebase.firestore().collection('users').get()
            .then(snapshot => {
                ...
                snapshot.forEach(doc => {
                    ...
                    li.textContent = `UID: ${doc.id}`;
                    userList.appendChild(li);
                });
            })
            .catch(error => {
                console.error("🔥 Lỗi khi đọc Firestore:", error);
                userList.innerHTML = "<li>Lỗi khi tải dữ liệu người dùng.</li>";
            });
    }
});
